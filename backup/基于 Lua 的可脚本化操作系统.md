...
==可脚本化操作系统主张：操作系统可以通过允许用户对其内核进行脚本化编程来充分提供可扩展性==。

本文还展示了一种 kernel-scripting environment（内核脚本化环境）的实现，该环境允许用户使用脚本语言 Lua 对 Linux 和 NetBSD 操作系统进行动态扩展。为评估该环境，我们对两种操作系统内核进行了扩展，使用户能够使用 Lua 对 CPU frequency scaling（CPU 频率调节）和 network packet filtering（网络数据包过滤）进行脚本化编程。
