...
1. 支持未修改的应用二进制程序是至关重要的，否则用户将不会迁移到 Xen。因此，我们必须虚拟化现有标准 ABI 所要求的所有架构特性。

2. 支持完整的多应用操作系统非常重要，因为这允许在单个客户操作系统实例中虚拟化复杂的服务器配置。

3. 在 x86 等不合作的（uncooperative）机器架构上，准虚拟化对于获得高性能和强资源隔离是必要的。

4. 即使在合作的机器架构上，完全隐藏资源虚拟化的影响也会在正确性和性能两方面带来风险。
...
[^1]: 例如，线程库经常使用段来寻址线程本地数据。
[^2]: 事后来看，将值写入一个预先约定的共享内存位置而非修改栈帧会简化 XP 的移植工作。
...
<!-- obsidian-publish-source: Operating System 操作系统/Pappers 论文/SOSP/Xen 和 虚拟化的艺术.md -->